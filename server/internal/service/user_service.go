...
func (s *UserService) UpdateProfile(ctx context.Context, userID string, updates map[string]interface{}) (*model.UserVO, error) {
...
func (s *UserService) GetFavorites(ctx context.Context, userID string) ([]model.ArticleVO, error) {
	...
	vos := make([]model.ArticleVO, 0, len(favs))
	for _, f := range favs {
		...
		vos = append(vos, model.ArticleVO{
			ID:        a.ID,
			Title:     a.Title,
			Summary:   a.Summary,
			Author:    a.Author,
...
func (s *UserService) GetAchievements(ctx context.Context, userID string) ([]model.AchievementVO, error) {
	// 获取用户已解锁的成就
	...
	vos := make([]model.AchievementVO, 0, len(allAchs))
	for _, a := range allAchs {
		vo := model.AchievementVO{
			ID:          a.ID,
			Name:        a.Name,
...
// GetQuests 获取用户每日任务
func (s *UserService) GetQuests(ctx context.Context, userID string) ([]model.QuestVO, error) {
	// 获取所有任务定义
	...
	vos := make([]model.QuestVO, 0, len(allQuests))
