  ...
  <div class="min-h-screen">
    ...
    <section class="relative pt-24 pb-8" style="background: var(--color-bg-primary);">
      <div class="max-w-7xl mx-auto px-6">
        ...
        <h1 class="text-2xl md:text-3xl font-bold mb-6" style="font-family: var(--font-display); color: var(--color-text-primary); word-break: keep-all;">{{ $t('search.title') }} // {{ $t('search.subtitle') }}</h1>
        ...
        <div class="relative max-w-2xl">
          <div class="flex items-center rounded-md overflow-hidden transition-all duration-150" style="background: var(--color-bg-card); border: 1px solid var(--color-border);">
            ...
            <input type="text" v-model="searchQuery" class="flex-1 px-4 py-3 text-base font-medium outline-none" style="background: transparent; color: var(--color-text-primary); font-family: var(--font-body); caret-color: var(--color-primary);" :placeholder="$t('search.placeholder')" />
            <span class="shrink-0 px-4 py-3 text-sm" style="font-family: var(--font-mono); color: var(--color-primary);">{{ $t('search.results', { count: searchResults.length }) }}</span>
          </div>
      ...
      <div class="grid gap-8" :style="{ gridTemplateColumns: '1fr 300px' }">
        ...
        <div>
          <div class="flex items-center gap-3 mb-5">
            ...
            <h2 class="text-lg font-semibold truncate" style="font-family: var(--font-display); color: var(--color-text-primary);">{{ $t('search.scanResults') }}</h2>
          ...
          >
            <div class="p-5">
              ...
              <div class="flex items-center gap-3 text-xs" style="color: var(--color-text-tertiary);">
                <span>{{ $t('search.source') }}{{ result.source }}</span>
...
const getCategoryColor = (category: any) => {
  const slug = getCatSlug(category)
